...
$$
u\left(t\right)=K_pe\left(t\right)+K_i\int e\left(t\right)dt+K_d\frac{de(t)}{dt}
$$
...
$$
u\left(k\right)=K_pe\left(k\right)+K_i\sum_{j=1}^{k}e\left(j\right)+K_d\left[e\left(k\right)-e\left(k-1\right)\right]
$$
而增量式PID算法对应可由位置式推出：
...
if (pid->errABS > 50) {
    ...
    kiIndex = 0.600f;
} else if(pid->errABS > 25 & pid->errABS < 30) {
    kiIndex = 0.900f;
} else {
    kiIndex = 1.000f;
}
