...
使用 sudo apt list --installed | grep denyhosts 来搜索会有警告apt does not have a stable CLI interface. 因为apt 的输出是为用户(人)设计的，并非总能被其它命令行工具解析，比如它会有个进度条和颜色等交互界面，而这个进度条会影响其它命令行工具解析它的输出。因此不要在脚本中使用apt命令，应该用apt-get、apt-cache等命令进行替换。
